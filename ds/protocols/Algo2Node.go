...
func (node *Algo2Node) Initialize(id int, name string, mlp ml.MLProcess, net network.Network[Algo2Message], heartbeatNet network.Network[Algo2Message], numNodes int) {
	node.id = id
	node.name = name
	node.ml = mlp
	node.net = net
